...
func TestStackparseAssignPC(t *testing.T) {
	cases := []struct {
		name     string
		trace    [addrspace.StackDepth]uint64
		expected string
	}{
		{
			name:     "only libc, one frame",
			trace:    [addrspace.StackDepth]uint64{0x7ffff75b0000}, // stack traces come in 0-padded lengths of powers of two
			expected: "/usr/lib/x86_64-linux-gnu/libc.so.6",
		},
		{
			name:     "only libc, two frames",
			trace:    [addrspace.StackDepth]uint64{0x7ffff75b0000, 0x7ffff75b0004},
			expected: "/usr/lib/x86_64-linux-gnu/libc.so.6",
		},
		{
			name:     "libssl via libc",
			trace:    [addrspace.StackDepth]uint64{0x7ffff75b0000, 0x7ffff75b0004, 0x7ffff7e33000},
			expected: "/usr/lib/x86_64-linux-gnu/libssl.so.3",
		},
		{
			name:     "anonymous, one frame",
			trace:    [addrspace.StackDepth]uint64{0x7ffff7fbd000},
			expected: "anonymous",
		},
		{
			name:     "anonymous via libc",
			trace:    [addrspace.StackDepth]uint64{0x7ffff75b0000, 0x7ffff7fbd000},
			expected: "anonymous",
		},
		{
			name:     "nginx via libc",
			trace:    [addrspace.StackDepth]uint64{0x7ffff75b0000, 0x55555567c000},
