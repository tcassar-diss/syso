...
// OldTracer gathers information about system call statistics by dynamic execution
type OldTracer interface {
	// Trace records all system calls for the given executable and its arguments
	Trace(binPath string, args ...string) error
}

func NewOldTracer(logger *zap.SugaredLogger, statsList []Stat) OldTracer {
	return &oldTracer{
		logger: logger,
		stats:  statsList,
...
func (t *oldTracer) Trace(binPath string, args ...string) error {
	pid := os.Getpid()
	tgid := os.Getgid()
	...
	if err := t.listen(rd, objs.ScEventsFullMap); err != nil {
...
func (t *oldTracer) DumpStats(fp string) error {
...
func (t *oldTracer) isElf(fp string) (bool, error) {
	f, err := os.Open(fp)
	if err != nil {
...
func (t *oldTracer) listen(rd *ringbuf.Reader, maps *ebpf.Map) error {
